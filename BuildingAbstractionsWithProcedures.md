...
「引数を評価し(てから)、作用する」ので、  
まず0を評価→問題なし  
次に(p)を評価(＝展開？)しようとして、(p)を再び得る→ループ  
(つまり、testの作用の前にループしちゃう)  
by https://github.com/gx-hackers/SICP/pull/3/files#r8930990  
...
==> 0                                       ;; 条件節がtrueになったので  
                                            ;; then節の 0 を値として返す  
```
by https://github.com/gx-hackers/SICP/pull/3#issuecomment-32487130  
### 正規順序の評価を使う解釈系の場合  
正規順序では、「完全に展開し、簡約する」ので  
最初にtestを展開しようとする  
その時にifが動く(評価される)のであれば、最初の引数0(→x)を評価して、0が返っちゃって完了。  
by https://github.com/gx-hackers/SICP/pull/3/files#r8931078  
