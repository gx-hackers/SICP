...
* 違う部分
 * 反復では、どの時点においても一定数のプログラム変数によりプロセスの状態完全な記述を持つ（プロセス実行のために必要なのはそれらの変数情報のみ）
...
wikipediaの例だと、Ack(x,y)でx=0の時の値はy+1となっているが、この例では2yとなっている。
アッカーマン関数には何種類かあるらしいので、その一種ってこと？
...
* 正の整数nに対して、手続きf,g,hが計算する関数の数学的な定義を述べよ
